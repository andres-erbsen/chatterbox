...
func sovDenameChatProfile(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
...
func sozDenameChatProfile(x uint64) (n int) {
	return sovDenameChatProfile(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
...
type randyDenameChatProfile interface {
	Float32() float32
	Float64() float64
	Int63() int64
	Int31() int32
...
func randStringDenameChatProfile(r randyDenameChatProfile) string {
	...
	tmps := make([]rune, v5)
	for i := 0; i < v5; i++ {
		tmps[i] = randUTF8RuneDenameChatProfile(r)
	}
	return string(tmps)
}
func randUnrecognizedDenameChatProfile(r randyDenameChatProfile, maxFieldNumber int) (data []byte) {
	l := r.Intn(5)
	for i := 0; i < l; i++ {
		wire := r.Intn(4)
		if wire == 3 {
		...
		data = randFieldDenameChatProfile(data, r, fieldNumber, wire)
	}
	return data
}
func randFieldDenameChatProfile(data []byte, r randyDenameChatProfile, fieldNumber int, wire int) []byte {
	key := uint32(fieldNumber)<<3 | uint32(wire)
	switch wire {
	case 0:
		data = encodeVarintPopulateDenameChatProfile(data, uint64(key))
		v6 := r.Int63()
		if r.Intn(2) == 0 {
			v6 *= -1
		}
		data = encodeVarintPopulateDenameChatProfile(data, uint64(v6))
	case 1:
		data = encodeVarintPopulateDenameChatProfile(data, uint64(key))
	...
	case 2:
		data = encodeVarintPopulateDenameChatProfile(data, uint64(key))
		ll := r.Intn(100)
		data = encodeVarintPopulateDenameChatProfile(data, uint64(ll))
		for j := 0; j < ll; j++ {
...
func encodeVarintPopulateDenameChatProfile(data []byte, v uint64) []byte {
	for v >= 1<<7 {
...
func encodeFixed32DenameChatProfile(data []byte, offset int, v uint32) int {
	data[offset] = uint8(v)
	data[offset+1] = uint8(v >> 8)
	data[offset+2] = uint8(v >> 16)
	data[offset+3] = uint8(v >> 24)
...
func encodeVarintDenameChatProfile(data []byte, offset int, v uint64) int {
	for v >= 1<<7 {
