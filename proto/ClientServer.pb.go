...
func (m *ClientToServer) Unmarshal(data []byte) error {
	...
	for index < l {
		...
		case 11:
			...
			m.GetNumKeys = &Byte32{}
			if err := m.GetNumKeys.Unmarshal(data[index:postIndex]); err != nil {
				return err
			}
			index = postIndex
		default:
...
func (m *ClientToServer) MarshalTo(data []byte) (n int, err error) {
	...
	if m.GetNumKeys != nil {
		data[i] = 0x5a
		i++
		i = encodeVarintClientServer(data, i, uint64(m.GetNumKeys.Size()))
...
func (this *ClientToServer) Equal(that interface{}) bool {
	...
	if that1.GetNumKeys == nil {
		if this.GetNumKeys != nil {
			return false
		}
	} else if !this.GetNumKeys.Equal(*that1.GetNumKeys) {
		return false
	}
	if !bytes1.Equal(this.XXX_unrecognized, that1.XXX_unrecognized) {
		return false
