...
func NewPopulatedServerToClient(r randyClientServer, easy bool) *ServerToClient {
	...
	if r.Intn(10) != 0 {
		this.MessageId = NewPopulatedByte32(r)
	}
	if r.Intn(10) != 0 {
		v6 := r.Int63()
		if r.Intn(2) == 0 {
...
func NewPopulatedClientToServer(r randyClientServer, easy bool) *ClientToServer {
	...
	if r.Intn(10) != 0 {
		v11 := r.Intn(100)
		this.UploadSignedKeys = make([][]byte, v11)
		for i := 0; i < v11; i++ {
			v12 := r.Intn(100)
			this.UploadSignedKeys[i] = make([]byte, v12)
			for j := 0; j < v12; j++ {
...
func randFieldClientServer(data []byte, r randyClientServer, fieldNumber int, wire int) []byte {
	...
	case 0:
		...
		data = encodeVarintPopulateClientServer(data, uint64(v17))
	case 1:
		data = encodeVarintPopulateClientServer(data, uint64(key))
...
func (m *ServerToClient) MarshalTo(data []byte) (n int, err error) {
	...
	if m.MessageId != nil {
		data[i] = 0x32
		i++
		i = encodeVarintClientServer(data, i, uint64(m.MessageId.Size()))
		n1, err := m.MessageId.MarshalTo(data[i:])
		if err != nil {
			return 0, err
		}
		i += n1
...
func (m *ClientToServer) MarshalTo(data []byte) (n int, err error) {
	...
	if m.GetSignedKey != nil {
		...
		n4, err := m.GetSignedKey.MarshalTo(data[i:])
		if err != nil {
			return 0, err
		}
		i += n4
...
func (this *ServerToClient) Equal(that interface{}) bool {
	...
	if that1.MessageId == nil {
		if this.MessageId != nil {
			return false
		}
	} else if !this.MessageId.Equal(*that1.MessageId) {
		return false
	}
